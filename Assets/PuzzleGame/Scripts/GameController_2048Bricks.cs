...
public class GameController_2048Bricks : BaseGameController<GameState2048Bricks>
{
    [Header("Mode fields")]
    public float speed;
    ...
    void SpawnStartingBricks()
    {
        ...
        for (int i = 0; i < bricksCount.x; i++)
        {
            ...
            NumberedBrick brick = SpawnBrick(new Vector2Int(i, 0), numbers[rand]);
            brick.PointerClick += b => { OnHighlightedTargetClick(brick); };
            field[i, 0] = brick;
    ...
    void SpawnColumns()
    {
        for (int i = 0; i < bricksCount.x; i++)
        {
            ...
            RectTransform rect = columnBrick.RectTransform;
    ...
    protected override void OnLastChanceCompleted()
    {
        Normalize(
            normalized =>
            {
                ...
                {
                    currentBrickCoords = new Vector2Int(bricksCount.x / 2, bricksCount.y - 1);
                    ((GameState2048Bricks)gameState).CurrentBrickCoords = currentBrickCoords;
